...
const genIter = (node, nodeTransformer) => {
  ...
  while (stack.length) {
    ...
    switch (currentSnapshot.stage) {
      case 0:
        if (currentSnapshot.node && Array.isArray(currentSnapshot.node.child)) {
          const curr = {}
          if (currentSnapshot.node.opt === 'OR') {
            curr.should = []

            currentSnapshot.node.child.forEach((element, i) => {
          ...
          } else if (currentSnapshot.node.opt === 'AND') {
            curr.must = []

            currentSnapshot.node.child.forEach(element => {
              if (Array.isArray(element.child)) return
          ...
          } else if (currentSnapshot.node.opt === 'NOT') {
            curr.must_not = []
          ...
          } else if (currentSnapshot.node.opt === 'NEAR' || currentSnapshot.node.opt === 'PRE') {
            curr.must = []

            const in_order = currentSnapshot.node.opt === 'PRE'

            let slop

            switch (currentSnapshot.node.span) {
              case 'S':
                slop = '15'
                break
              case 'P':
                ...
                break
              default:
                slop = currentSnapshot.node.span
            }
          ...
          currentSnapshot.current = curr
          currentSnapshot.next.ref = curr

          if (currentSnapshot.node.child[1] && currentSnapshot.node.child[1].child) {
            stack.push({ node: currentSnapshot.node, current: curr, next: {}, stage: 2 })
            stack.push({ node: currentSnapshot.node.child[1], next: stack[stack.length - 1].next, stage: 0 })
          }

          if (currentSnapshot.node.child[0] && currentSnapshot.node.child[0].child) {
            stack.push({ node: currentSnapshot.node, current: curr, next: {}, stage: 1 })
            stack.push({ node: currentSnapshot.node.child[0], next: stack[stack.length - 1].next, stage: 0 })
          }
        }
        break
      case 1:
        if (currentSnapshot.next.ref) {
          const ncKeys = Object.keys(currentSnapshot.next.ref)

          if (currentSnapshot.node.opt === 'AND' && ((ncKeys.length > 1) || (ncKeys.length === 1 && ncKeys[0] !== 'must'))) {
            currentSnapshot.current.must.push({ bool: currentSnapshot.next.ref })
          } else if (currentSnapshot.node.opt === 'AND' && ncKeys.length === 1 && ncKeys[0] === 'must') {
            const x = currentSnapshot.current.must.map(i => JSON.stringify(i))
            currentSnapshot.next.ref.must.forEach(s => {
              if (!x.includes(JSON.stringify(s))) {
                currentSnapshot.current.must.push(s)
              }
            })
          } else if (currentSnapshot.node.opt === 'OR' && ((ncKeys.length > 1) || (ncKeys.length === 1 && ncKeys[0] !== 'should'))) {
            currentSnapshot.current.should.push({ bool: currentSnapshot.next.ref })
          } else if (currentSnapshot.node.opt === 'OR' && ncKeys.length === 1 && ncKeys[0] === 'should') {
            const [prev = {}] = currentSnapshot.current.should
            let key
            if (prev.term) key = Object.keys(prev.term)[0]
            const x = currentSnapshot.current.should.map(i => JSON.stringify(i))
            currentSnapshot.next.ref.should.forEach(s => {
              if (s.terms && s.terms[key]) {
                ...
                if (!x.includes(JSON.stringify(s))) {
                  currentSnapshot.current.should.push(s)
                }
              }
            })
          } else if (currentSnapshot.node.opt === 'NOT') {
            const [prev = {}] = currentSnapshot.current.must_not
            let key
            if (prev.term) key = Object.keys(prev.term)[0]

            ncKeys.forEach(k => {
              if (!currentSnapshot.current[k]) currentSnapshot.current[k] = currentSnapshot.next.ref[k]
              else {
                const x = currentSnapshot.current[k].map(i => JSON.stringify(i))
                if (k === 'must_not' && key) {
                  prev.terms = {
                    [key]: [prev.term[key]]
                  }
                }
                currentSnapshot.next.ref[k].forEach(s => {
                  if (!x.includes(JSON.stringify(s))) {
                    ...
                    } else currentSnapshot.current[k].push(s)
                  }
                })
          ...
          } else if ((currentSnapshot.node.opt === 'NEAR' || currentSnapshot.node.opt === 'PRE') && ((ncKeys.length > 1) || (ncKeys.length === 1 && ncKeys[0] === 'must_not'))) {
            throw new Error('malformed query')
          } else if ((currentSnapshot.node.opt === 'NEAR' || currentSnapshot.node.opt === 'PRE') && ncKeys.length === 1 && ncKeys[0] === 'should') {
            currentSnapshot.current.must[0].span_near.clauses.push({
              span_or: {
                clauses: currentSnapshot.next.ref.should.reduce((previousValue, currentValue) => {
          ...
          } else if ((currentSnapshot.node.opt === 'NEAR' || currentSnapshot.node.opt === 'PRE') && ncKeys.length === 1 && ncKeys[0] === 'must') {
            currentSnapshot.next.ref.must.forEach(x => {
              if (x.term) {
                x.span_term = x.term
                delete x.term
              } else if (x.wildcard) {
            ...
            currentSnapshot.current.must[0].span_near.clauses.push(...currentSnapshot.next.ref.must)
          }
        }
        break
      case 2:
        if (currentSnapshot.next.ref) {
          ...
          if (currentSnapshot.node.opt === 'AND' && ((ncKeys.length > 1) || (ncKeys.length === 1 && ncKeys[0] !== 'must'))) {
            ...
            currentSnapshot.current.must_not.push({ bool: currentSnapshot.next.ref })
          } else if ((currentSnapshot.node.opt === 'NEAR' || currentSnapshot.node.opt === 'PRE') && ((ncKeys.length > 1) || (ncKeys.length === 1 && ncKeys[0] === 'must_not'))) {
            throw new Error('malformed query')
          } else if ((currentSnapshot.node.opt === 'NEAR' || currentSnapshot.node.opt === 'PRE') && ncKeys.length === 1 && ncKeys[0] === 'should') {
            currentSnapshot.current.must[0].span_near.clauses.push({
